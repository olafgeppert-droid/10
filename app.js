...
// NEUE DRUCKFUNKTIONEN für Windows/Mac
function printTable() {
$("#dlgPrint").close();
if (isIOS()) {
// iOS: Alte PDF-Lösung beibehalten
const el = resolvePrintableEl('#peopleTable');
elementToPdfBlob(el, 'Tabelle').then(blob => shareOrDownloadPDF(blob, 'tabelle.pdf'));
} else {
// Windows/Mac: Standard-Browser-Druckdialog
prepareForPrinting('table', () => {
window.print();
});
}
}
...
// Windows/Mac: Standard-Browser-Druckdialog
prepareForPrinting('tree', () => {
window.print();
});
}
}

function prepareForPrinting(type, callback) {
...
document.body.classList.add('printing-mode', `printing-${type}`);
...
setTimeout(() => {
document.body.classList.remove('printing-mode', `printing-${type}`);
}, 500);
}, 100);
}
...
// iOS-spezifische Druckfunktion (unverändert)
const originalStyles = document.querySelectorAll('style, link[rel="stylesheet"]');
originalStyles.forEach(el => el.setAttribute('media', 'not print'));
...
.ribbon, .toolbar, .hint-row, .table-hint, .version-under,
.dialog, .btn, .search-container {
...
setTimeout(() => {
originalStyles.forEach(el => el.setAttribute('media', 'all'));
printStyle.remove();
}, 1000);
}
